...
char *count[10000];
char *clipboard;
int len = 0 , pos_line , pos_char , Have_Space = 0 , counter_new_LINE , size , first_cur , second_cur , size_clipboard , space_add = 0;
char bORf ;
char cORI; 
...
void grep(){
    ...
    if(temp != '\"'){
        ...
        while(1){
            scanf("%c" , &matn[x]);
            if(matn[x] == '/'){
                printf("matn[x-1] is : %c\n" , matn[x-1]);
                printf("matn[x-2] is : %c\n" , matn[x-2]);
                printf("matn[x-3] is : %c\n" , matn[x-3]);
                if(matn[x-1] == ' '){
                    if(matn[x-2] == '\"'){
                        space_add = 0;
                        matn[x-2] = '\0';
